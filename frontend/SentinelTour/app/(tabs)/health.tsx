    ...
    <Svg width={width} height={height}>
      ...
      {data.map((v, i) => (
        <Circle key={`dot-${i}`} cx={toX(i)} cy={toY(v)} r="3"
          fill={color} stroke={Colors.surface} strokeWidth="1.5"
        />
      ))}
    </Svg>
  );
      ...
      >
        ...
        <View style={styles.metricCardHeader}>
          ...
          <View style={styles.metricRight}>
            {isAlert && <LivePulse color={Colors.error} />}
      ...
      >
          ...
          {device?.isConnected ? (
            <>
              ...
              <View style={styles.batteryWrap}>
                <Icon.Battery
                  size={18}
                  color={(device.batteryPercentage ?? 100) < 20 ? Colors.error : Colors.success}
                />
                <Text style={[
                  styles.batteryText,
                  { color: (device.batteryPercentage ?? 100) < 20 ? Colors.error : Colors.textSecondary },
                ]}>
            ...
            <>
              <View style={styles.deviceBannerLeft}>
                <View style={[styles.pulseDot, { backgroundColor: Colors.textMuted }]} />
                <View>
                  <Text style={styles.deviceBannerTitle}>No Device Connected</Text>
                  <Text style={styles.deviceBannerSub}>Data from last backend sync</Text>
                </View>
              </View>
              <Icon.WifiOff size={18} color={Colors.textMuted} />
            </>
          )}
        </Animated.View>
        ...
        {lastUpdated && (
          <Animated.View entering={FadeInDown.duration(300).delay(80)} style={styles.syncRow}>
            ...
            <Text style={styles.syncText}>Updated {lastUpdated}</Text>
        ...
        {!isLoading && latest && (
          <View style={styles.metricsSection}>
            ...
            <MetricDetailCard
              ...
              history={hrHistory}
              normal="60 – 100 bpm"
              isAlert={!!latest.is_alert && latest.alert_type === 'HEART_RATE'}
              index={0}
            />
            <MetricDetailCard
              ...
              history={spo2History}
              normal="95 – 100 %"
              isAlert={!!latest.is_alert && latest.alert_type === 'SPO2'}
              index={1}
            />
            <MetricDetailCard
              ...
              history={tempHistory}
              normal="36.1 – 37.2 °C"
              isAlert={!!latest.is_alert && latest.alert_type === 'TEMPERATURE'}
              index={2}
            />
            <MetricDetailCard
              label="Ambient Temperature"
              value={latest.ambient_temperature}
              unit="°C"
              icon={<Icon.Wind size={20} color={Colors.accent} />}
              color={Colors.accent}
              history={[]}
              normal="Environment reading"
              isAlert={false}
              index={3}
            />
          </View>
        ...
        {history.length > 0 && (
          <Animated.View entering={FadeInDown.duration(400).delay(400)} style={styles.statsSection}>
            ...
            <View style={styles.statsGrid}>
              ...
              <StatBox
                label="Min SpO₂"
                value={spo2History.length > 0 ? `${Math.min(...spo2History)}` : '—'}
                color={Colors.spo2}
                unit="%"
              />
              <StatBox
                label="Alerts"
                value={String(history.filter((h) => h.is_alert).length)}
                color={Colors.error}
              />
            </View>
          </Animated.View>
...
const styles = StyleSheet.create({
  ...
  deviceBannerTitle: { fontSize: Typography.sm, fontFamily: 'SpaceGrotesk_600SemiBold', color: Colors.textPrimary },
  deviceBannerSub:  { fontSize: Typography.xs, fontFamily: 'Inter_400Regular', color: Colors.textMuted, marginTop: 1 },
  ...
  syncText:    { fontSize: Typography.xs, fontFamily: 'Inter_400Regular', color: Colors.textMuted, flex: 1 },
  syncBtn:     { paddingHorizontal: Spacing.sm, paddingVertical: 3, backgroundColor: Colors.surface, borderRadius: Radius.sm, borderWidth: 1, borderColor: Colors.border },
  syncBtnText: { fontSize: Typography.xs, fontFamily: 'Inter_500Medium', color: Colors.primary },

  // Loading
  loadingBox: { paddingVertical: Spacing['3xl'], alignItems: 'center', gap: Spacing.md },
  loadingText: { fontSize: Typography.sm, fontFamily: 'Inter_400Regular', color: Colors.textMuted },
  ...
  metricCardAlert: { borderColor: 'rgba(239,68,68,0.4)' },
  ...
  metricLabel:  { fontSize: Typography.sm, fontFamily: 'SpaceGrotesk_600SemiBold', color: Colors.textPrimary },
  metricNormal: { fontSize: Typography.xs, fontFamily: 'Inter_400Regular', color: Colors.textMuted, marginTop: 1 },
  ...
  alertMsg:    { fontSize: Typography.xs, fontFamily: 'Inter_400Regular', color: Colors.error },
  chartWrap:   { gap: Spacing.sm },
  chartHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  chartTitle:  { fontSize: Typography.xs, fontFamily: 'Inter_500Medium', color: Colors.textSecondary },
  chartEmpty:  { paddingVertical: Spacing.md, alignItems: 'center' },
  chartEmptyText: { fontSize: Typography.xs, fontFamily: 'Inter_400Regular', color: Colors.textMuted },
  ...
  noDataTitle: { fontSize: Typography.lg, fontFamily: 'SpaceGrotesk_700Bold', color: Colors.textPrimary },
  noDataSub: {
  ...
  statLabel:    { fontSize: Typography.xs, fontFamily: 'Inter_400Regular', color: Colors.textMuted },
});