    ...
    searchTimeout.current = setTimeout(async () => {
      try {
        const res = await fetch(
          `${Config.NOMINATIM_URL}/search?format=json&q=${encodeURIComponent(text)}&limit=6&addressdetails=1${myLocation ? `&lat=${myLocation.latitude}&lon=${myLocation.longitude}` : ''}`,
